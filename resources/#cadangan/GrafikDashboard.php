...
class GrafikDashboard extends Component
{
    ...
    public function mount(){
        ...
        if($this->userAdmin || $this->userTerapis->is_kepala) {
            $this->dataGrafik = $this->grafikPerTahun($this->grafik, Carbon::now()->year, '', '');$this->listPenyakit = SubRekamMedis::distinct('penyakit')->orderBy('penyakit', 'ASC')->pluck('penyakit');            
        } else {            
    ...
    {
        if($this->userAdmin || $this->userTerapis->is_terapis) {
    ...
    public function setFilterAction($filter) {
        ...
        $namaPenyakit = $this->nama_penyakit;

        $scopeTerapis = $this->nama_penyakit == null ? $this->listTerapis : 
                        Terapis::whereHas('subRekamMedis', function ($query) use ($namaPenyakit) {
                                $query->where('penyakit', $namaPenyakit);                            
